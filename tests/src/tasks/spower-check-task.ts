...
import { FileInfoV2, Replica } from '../types/chain';
import { FilesRecord, GroupMembersRecord, SworkerKeysRecord } from '../types/database';
  ...
  do {
    try {
        ...
        do {
          ...
          for (const [_cid, fileInfoV2] of fileInfoV2MapChain) {
        ...
        for (const [anchor, workReport] of allWorkReportsOnChain) {
          const workReportReportSlot = workReport.report_slot;
